...
public class Util {
     ...
     * @param ma
     * @param mb
     * @param operatorB 在第mb矩阵上的操作
     * @param operatorA 在ma矩阵元素上的操作
     * @return
     * 
     */
    public static double[][] matrixOp(final double[][] ma, final double[][] mb,
        ...
        if (m != mb.length || n != mb[0].length)
            throw new RuntimeException("两个矩阵大小不一致 ma.length:" + ma.length + "  mb.length:"
                    + mb.length);
    ...
    public static double[][] scaleMatrix(final double[][] matrix, final Size scale) {
        ...
        if (sm * scale.x != m || sn * scale.y != n) throw new RuntimeException("scale不能整除matrix");
    ...
    public static double[][] convnValid(final double[][][][] matrix, int mapNoX,
        ...
        int kns = n - kn + 1;
        int khs = h - kh + 1;
        if (matrix.length != kernel.length) throw new RuntimeException("矩阵与卷积核在第一维上不同");
        // 结果矩阵
